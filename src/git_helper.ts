...
export class GitHelper {
    ...
    public async getDiffWithHead(commit?: string): Promise<String[] | null> {
        ...
        try {
            ...
            const diffs = await repository.diffBlobs("28c7b10e93626065d02e56eaf2e3754a18f59821", 'HEAD');
    ...
    function parseDiff(diff: string): DiffResult {
        ...
        diffLines.forEach(line => {
            if (line.startsWith('@@')) {
                ...
                if (line.startsWith('+')) {
                    addedLines.push({ line: line.substring(1).trim(), lineNumber: currentNewLineNumber });
                    currentNewLineNumber++;
                } else if (line.startsWith('-')) {
                    removedLines.push({ line: line.substring(1).trim(), lineNumber: currentOldLineNumber });
